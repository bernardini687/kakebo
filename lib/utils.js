...
function formatDate (date) {
  ...
  // const [m, y] = [now.month() === target.month(), now.year() === target.year()]
  ...
  // } else if (y) {
  //   return target.format(format + 'MMM')
  // }

  return target.format(format + 'MMM Y')
}
...
function prettify (expenses) {
  ...
  return expenses.map(({ amount, date, tag }) => {
    return formatDate(date).padEnd(15, ' ') +
      fromCents(amount).padStart(maxAmount + 2, ' ') + '  ' +
